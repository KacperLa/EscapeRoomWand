...
void ODriveUART::setPosition(const int axis, float position) {
    setPosition(axis, position, 0.0f, 0.0f);
}

void ODriveUART::setPosition(const int axis, float position, float velocity_feedforward) {
    setPosition(axis, position, velocity_feedforward, 0.0f);
}

void ODriveUART::setPosition(const int axis, float position, float velocity_feedforward, float torque_feedforward) {
...
void ODriveUART::setState(const int axis, ODriveAxisState requested_state) {
    setParameter("axis" + (String)axis + ".requested_state", String((long)requested_state));
}

ODriveAxisState ODriveUART::getState(const int axis) {
    return (ODriveAxisState)getParameterAsInt("axis" + (String)axis + ".current_state");
}
