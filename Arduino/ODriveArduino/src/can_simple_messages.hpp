...
struct Set_gpio_msg_t final {
    constexpr Set_gpio_msg_t() = default;

#ifdef ODRIVE_CAN_MSG_TYPE
    Set_gpio_msg_t(const TBoard::TCanIntf::TMsg& msg) {
        decode_msg(msg);
    }
    ...
    void encode_buf(uint8_t* buf) const {
        can_set_signal_raw<uint8_t>(buf, request_type, 0, 8, true);
        can_set_signal_raw<uint8_t>(buf, gpio_num, 8, 8, true);
        can_set_signal_raw<bool>(buf, state, 16, 1, true);
    }

    void decode_buf(const uint8_t* buf) {
        request_type = can_get_signal_raw<uint8_t>(buf, 0, 8, true);
        gpio_num = can_get_signal_raw<uint8_t>(buf, 8, 8, true);
        state = can_get_signal_raw<bool>(buf, 16, 1, true);
    }
